// Send driver profile email to employer
import { sendEmail } from '../lib/email.js';

const AIRTABLE_API_KEY = process.env.AIRTABLE_API_KEY;
const AIRTABLE_BASE_ID = process.env.AIRTABLE_BASE_ID;
const CANDIDATES_TABLE_ID = process.env.AIRTABLE_CANDIDATES_TABLE_ID;
const EMPLOYERS_TABLE_ID = process.env.AIRTABLE_EMPLOYERS_TABLE_ID;
...
function generateEmailHtml({ candidate, submission, pdfUrl, customMessage }) {
  ...
  const portfolioUrl = portfolio_slug ? `${APP_URL}/portfolio/${portfolio_slug}?submission=${submission.id}` : null;
        ...
        <table width="100%" cellpadding="0" cellspacing="0" style="max-width: 600px; background-color: #FFFFFF; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 24px rgba(0, 71, 81, 0.08);">
                  ...
                  <td style="padding: 14px 20px;">
                    <table width="100%" cellpadding="0" cellspacing="0">
                      <tr>
                        ...
                        <td align="right" style="color: ${BRAND.teal}; font-size: 28px; font-weight: 800;">${fitScore}%</td>
                      </tr>
                    </table>
                  </td>
                </tr>
          ...
          <tr>
            <td style="padding: 0 28px 28px;" align="center">
              <a href="${pdfUrl}" style="display: inline-block; background: ${BRAND.teal}; color: #FFFFFF; padding: 14px 36px; text-decoration: none; border-radius: 8px; font-weight: 600; font-size: 15px;">
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Get candidate UUID from submission
    ...
    const candidateUrl = `https://api.airtable.com/v0/${AIRTABLE_BASE_ID}/${CANDIDATES_TABLE_ID}?filterByFormula=${formula}&maxRecords=1`;
    ...
    const candidate = {
      uuid: candidateUuid,
      ...candidateData.records[0].fields,
    };

    // Use stored R2 PDF URL if available, otherwise fall back to on-demand generation
    ...
    const recipients = [toEmail, ccEmail].filter(Boolean).join(', ');
    res.status(200).json({
      success: true,
...
async function handlePreview(req, res) {
  ...
  if (!submissionId) {
    return res.status(400).send('submissionId query param required. Try: /api/submissions/send-email?submissionId=xxx');
  }
  ...
  try {
    ...
    const candidateUuid = submission.candidate_uuid;
    if (!candidateUuid) {
      return res.status(400).send('Submission has no candidate_uuid');
    }
